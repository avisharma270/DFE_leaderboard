...
GSHEET_URL = "https://docs.google.com/spreadsheets/d/11n6G7t9xI6JgbLZP6ErGw0iPoCIaxB1j5SlO8QJFYmQ/edit?usp=sharing"
...
with st.sidebar:
    ...
    view = st.radio(
        "Choose a view",
        ["My Performance", "Pan India Leaderboard", "Cohort Basis Leaderboard", "Zone-wise Leaderboard"],
...
if view == "My Performance":
    st.markdown("<div class='sec-title'>My Performance</div>", unsafe_allow_html=True)
    st.caption("Find yourself to see your rank across the Pan India, Cohort and Zone leaderboards.")

    label_map = {
        f"{r['emp_code']}  -  {r['name']}".strip(" -"): r["emp_code"]
        for r in board[["emp_code", "name"]].to_dict("records")
    }
    choice = st.selectbox("Search by name or ID",
                          options=["-- select your name or ID --"] + sorted(label_map.keys()),
                          index=0)
    selected_id = label_map.get(choice, "")

    if selected_id and (board["emp_code"] == selected_id).any():
        me = board[board["emp_code"] == selected_id].iloc[0].to_dict()
        key = me["tier"]
        tier_size = int((board["tier"] == key).sum())
        zone_size = int((board["zone"] == me["zone"]).sum())

        # rank + performance card (Pan India / Cohort / Zone ranks)
        st.markdown(me_card_html(me, n_comp, tier_size, zone_size), unsafe_allow_html=True)

        # ongoing streaks
        st.markdown(streak_card_html(me), unsafe_allow_html=True)
        with st.expander("How streaks turn into points"):
            st.markdown(
                f"A **streak** is doing the same task on consecutive days. You need "
                f"**{STREAK_MIN_DAYS} days in a row** before any streak points start; miss a day and it resets.\n\n"
                f"**KYC streak** — during the first {STREAK_MIN_DAYS} qualifying days each *extra* KYC after the "
                f"first that day earns **+{KYC_50_PTS} pt** (Rs.50). From day {STREAK_MIN_DAYS + 1} onward the 1st "
                f"KYC earns **+{KYC_50_PTS} pt** (Rs.50) and every extra earns **+{KYC_100_PTS} pts** (Rs.100).\n\n"
                f"**Referral streak** — during the first {STREAK_MIN_DAYS} qualifying days each *extra* referral "
                f"earns **+{OB_PARTA_PTS} pt** (Part A); from day {STREAK_MIN_DAYS + 1} onward **every** referral "
                f"earns **+{OB_PARTB_PTS} pts** (Part B). Under-performers are not eligible.\n\n"
                f"Your tally: **{int(me['kyc_streak_pts'])}** KYC + **{int(me['ob_streak_pts'])}** Referral streak "
                f"pts = **{int(me['streak_pts'])}**, on top of **{int(me['base_points'])}** task points."
            )
        st.markdown(LEGEND, unsafe_allow_html=True)

        # day-by-day tracker
        st.markdown("<div class='sec-title'>Your July day-by-day</div>", unsafe_allow_html=True)
        st.caption("Bars = KYC / Referral / Retrofit per day. Ribbons show a streak building (days 1-3) "
                   "or active (day 4+); a gap breaks it.")
        kyc_d, onb_d, ret_d = get_daily_activity()
        st.markdown(daily_tracker_html(selected_id, kyc_d, onb_d, ret_d,
                                       pd.Timestamp(LIVE_FROM), as_of or pd.Timestamp(LIVE_FROM)),
                    unsafe_allow_html=True)

        # Pan India leaderboard (highlighted)
        st.markdown("<div class='sec-title'>Pan India Leaderboard &mdash; all cohorts combined</div>",
                    unsafe_allow_html=True)
        st.caption(f"Every DFE ranked together by total points. {n_comp} DFEs.")
        st.markdown(leaderboard_html(board.sort_values("overall_rank"), "overall_rank", selected_id),
                    unsafe_allow_html=True)

        # Cohort leaderboard (highlighted)
        st.markdown(f"<div class='sec-title'>Cohort Leaderboard &mdash; {_esc(key)}</div>", unsafe_allow_html=True)
        st.caption(f"{tier_size} DFEs in the {key} cohort.")
        coh = board[board["tier"] == key].sort_values("cohort_rank")
        st.markdown(leaderboard_html(coh, "cohort_rank", selected_id), unsafe_allow_html=True)

        # Zone leaderboard (highlighted)
        st.markdown(f"<div class='sec-title'>Zone Leaderboard &mdash; {_esc(me['zone'])}</div>",
                    unsafe_allow_html=True)
        st.caption(f"{zone_size} DFEs in {me['zone']} (all cohorts).")
        zl = board[board["zone"] == me["zone"]].sort_values("zone_rank")
        st.markdown(leaderboard_html(zl, "zone_rank", selected_id), unsafe_allow_html=True)
    else:
        st.info("Select your name or ID above to see your rank, points and streaks.")

# ---------------------------------------------------------------- Pan India
elif view == "Pan India Leaderboard":
    st.markdown("<div class='sec-title'>Pan India Leaderboard &mdash; all cohorts combined</div>",
                unsafe_allow_html=True)
...
# ---------------------------------------------------------------- Cohort
elif view == "Cohort Basis Leaderboard":
